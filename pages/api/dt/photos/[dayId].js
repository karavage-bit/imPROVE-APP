...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    const { data: row } = await supabase.from('dt_photos').select('storage_path').eq('student_id', user.id).eq('day_id', dayId).maybeSingle();
  ...
  if (req.method === 'POST') {
    ...
    if (buf.length > MAX_BYTES) return res.status(400).json({ error: `Photo exceeds ${MAX_BYTES / 1024}KB limit` });
    ...
    await supabase.from('dt_photos').upsert({ student_id: user.id, day_id: dayId, storage_path: storagePath, bytes: buf.length }, { onConflict: 'student_id,day_id' });
