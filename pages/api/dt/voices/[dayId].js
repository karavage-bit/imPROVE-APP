...
// GET    — returns { url, duration }
// POST   — accepts { base64: '...', mimeType: 'audio/webm', durationSeconds: 42 }
// DELETE — removes the voice memo
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    if (buf.length > MAX_BYTES)
      return res.status(400).json({ error: `Voice memo exceeds ${MAX_BYTES / 1024}KB limit` });
    ...
    await supabase.from('dt_voices').upsert(
      {
        student_id: user.id,
        day_id: dayId,
