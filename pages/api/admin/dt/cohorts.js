...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    const { data, error } = await supabase.from('dt_cohorts').select('*').order('created_at', { ascending: false });
  ...
  const { data, error } = await supabase.from('dt_cohorts').insert({ name, instructor_id: user.id, start_date: startDate ?? null, end_date: endDate ?? null, atlas_unlock_date: atlasUnlockDate ?? null, atlas_unlock_threshold: atlasUnlockThreshold ?? 24 }).select().single();
  if (error) return res.status(500).json({ error: error.message });
