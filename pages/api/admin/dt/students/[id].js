...
// GET /api/admin/dt/students/:id
// Full student detail: entries + media signed URLs + flags + earned moments
export default async function handler(req, res) {
  ...
  const [photoSigned, voiceSigned] = await Promise.all([
    Promise.all((photos ?? []).map(async p => {
      const { data } = await supabase.storage.from(BUCKET_PHOTOS).createSignedUrl(p.storage_path, SIGNED_TTL);
      return { dayId: p.day_id, url: data?.signedUrl ?? null };
    })),
    Promise.all((voices ?? []).map(async v => {
      const { data } = await supabase.storage.from(BUCKET_VOICES).createSignedUrl(v.storage_path, SIGNED_TTL);
      return { dayId: v.day_id, url: data?.signedUrl ?? null, duration: v.duration_seconds };
    })),
  ]);
  ...
  const stream = (entries ?? []).map(e => ({
    dayId: e.day_id,
    dayTitle: dayMeta.get(e.day_id)?.title ?? e.day_id,
    dayDate: dayMeta.get(e.day_id)?.date_label ?? '',
    region: dayMeta.get(e.day_id)?.region ?? '',
    ...
    photoUrl: photoUrlMap.get(e.day_id) ?? null,
    voice: voiceUrlMap.get(e.day_id) ?? null,
  }));
